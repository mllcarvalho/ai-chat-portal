 ...
 *
 * Arquivo único em CommonJS, sem o prefixo `node:` nos módulos nativos, para
 * que rode até em Node antigo o suficiente para exibir o aviso de versão
 * abaixo — em vez de quebrar com "Cannot find module 'node:url'".
 * É o scripts/setup.mjs do repositório sem os passos de build.
 */

// ---------- 0. trava de versão (precisa vir antes de qualquer require nativo) ----------

var nodeMajor = parseInt(process.versions.node.split('.')[0], 10);
if (nodeMajor < 18) {
  console.error(
    '\x1b[31m✗ Node.js ' + process.versions.node + ' é antigo demais para o instalador.\x1b[0m',
  );
  console.error('  Instale o Node.js 18 ou mais novo (versão LTS) em https://nodejs.org e rode o comando de novo.');
  console.error('  Para conferir a versão atual:  node --version');
  process.exit(1);
}
